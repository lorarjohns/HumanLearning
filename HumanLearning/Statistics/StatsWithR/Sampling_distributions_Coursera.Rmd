...
binwidth <- sample_means50 %>%
  summarise(bw = sd(price)*( 1/(5000^(1/3)))) %>%
  summarise(mean(bw)*3.49)
ggplot(sample_means50, aes(x=price)) +
...
binwidth <- sample_means150 %>%
  summarise(bw = sd(price)*( 1/(5000^(1/3)))) %>%
  summarise(mean(bw)*3.49)
ggplot(sample_means150, aes(x=price)) +
  geom_histogram(binwidth = binwidth[[1]])
```
